...
- **RL metrics**: `runs/rl_metrics.json` with mean episodic return ± std and `action_histogram` (% time at each NPI level).

Tiny summary table (demo, synthetic):

| Component | MAE | RMSE | Mean return ± std | Policy stability |
|---|---:|---:|---:|---:|
| GNN-LSTM + DQN | 0.12 | 0.23 | -1.8 ± 0.4 | {0:60%,1:25%,2:10%,3:5%} |
